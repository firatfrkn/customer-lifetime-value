...
LOGO = "C:/Users/furka/Masaüstü/firatli_photo.png"
...
# Slider Marks
x = np.linspace(
    df["spend_actual_vs_pred"].min(), df["spend_actual_vs_pred"].max(), 10, dtype=int
)
x = x.round(0)
    ...
    [
        html.A(
            ...
            dbc.Row(
                [
                    dbc.Col(
                        dbc.NavbarBrand("Customer Spend Prediction", className="ml-2")
                    ),
                ],
                align="center",
            ),
            ...
            [
                dbc.Col(
                    [
                        ...
                        html.P(
                            "Segment Customers that were predicted to spend but didn't. Then target these customers with targeted emails."
                        ),
                        dcc.Slider(
                            id="spend-slider",
                        ...
                        html.Br(),
                        html.Button("Download Segmentation", id="btn"),
                        dcc.Download(id="download"),
                    ],
                    width=3,
                    style={"margin": "10px"},
                ),
                dbc.Col(dcc.Graph(id="graph-slider"), width=8),
            ]
        ),
    ]
)
...
# CALLBACKS
@app.callback(Output("graph-slider", "figure"), Input("spend-slider", "value"))
